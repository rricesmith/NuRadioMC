...
def write_events_to_hdf5(filename, data_sets, attributes, n_events_per_file=None,
    ...
    while True:
        ...
        if(len(evt_ids_this_file) == 0):
            logger.debug("no more events to write in file {}".format(iFile))
            break
...
#                 stop_index = tmp[0]
        ...
        logger.info('writing file {} with {} events (id {} - {}) and {} entries'.format(filename2, n_events_this_file, evt_id_first,
...
def generate_eventlist_cylinder(filename, n_events, Emin, Emax,
    ...
    if proposal:
        ...
        logger.info(f"{np.sum(mask):.2e} taus ({np.sum(mask)/len(mask)*100:.1f}%) are created in nu tau interactions -> checking if tau decays in fiducial volume")
        n_taus = 0
        n_events_fiducial = 0
        for event_id in data_sets["event_ids"]:
            iE = event_id - start_event_id
        ...
        logger.info(f"added {n_taus:d} tau decays to the event list -> {n_taus/n_events_fiducial*100:.1f}% of events in fiducial volume")
