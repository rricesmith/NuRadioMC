...
with threadpool_limits(limits=1):
    ...
    class ray_tracing_2D():
        ...
        def get_attenuation_along_path(self, x1, x2, C_0, frequency, max_detector_freq, reflection=0, reflection_case=1):
            """
            calculates the frequency dependent attenuation factor that needs to be multiplied with the 
            (amplitude) frequency spectrum. Only exponential attenuation is included (NOT the 1/R signal falloff)
            ...
            x2: 3-dim array
                stop position of ray
            C_0: float
                ray tracing parameter C_0
            frequency: array of floats
                the frequencies the attenuation is calculated for
            max_detector_freq: float
                the maximum frequency of the detector (typically the Nquist frequency). This parameters is used
                for an efficient interpolation of the attenuation lenght. 
                
            Returns
            --------
            array of floats the same length as "frequency"
            """
